...
def get_energy(dataset: str, paths_dict:dict) -> dict:
    ...
    for line in lines:        
        split = line.split()
        if len(split) < 2:
            raise ValueError(f"Dataset '{dataset}' has a line without enough elements: '{line.strip()}'")
        try:
            ener_dict[split[0]] = float(split[1])
        except ValueError:
            raise ValueError(f"Dataset '{dataset}' has a line with an invalid energy value: '{line.strip()}'")
    return ener_dict
...
def get_graph_formula(graph: Data,
    ...
    """ 
    Create a string label for the selected graph.给定的图对象创建一个化学式的字符串标签
...
def structure_to_graph(geometry_file: str,
    ...
    return Data(x=x, edge_index=edge_index, species_list=species_list)
