...
from gnn_eads.constants import FULL_ELEM_LIST, MOL_ELEM, ENCODER, ELEMENT_LIST, RGB_COLORS, CORDERO
...
def plotter(graph: Data,
    ...
    node_sizes = [CORDERO[atom] * 100 for atom in labels.values()]  
    print(node_sizes)
    mean_node_size = sum(node_sizes) / len(node_sizes)  
    scaled_node_sizes = [size / mean_node_size * node_size for size in node_sizes]   
    # print(scaled_node_sizes)
    
    # font_sizes = {node: int(scaled_node_sizes[node] * font_size_ratio) for node in labels}

    plt.figure(figsize=figsize, dpi=dpi) 

    pos = kamada_kawai_layout(graph)  # 计算节点位置
    nx.draw_networkx(
        graph,
        pos=pos,
        labels=labels,
        node_size=[scaled_node_sizes[node] for node in graph.nodes()],  
        # font_size=[font_sizes[node] for node in graph.nodes()],  
