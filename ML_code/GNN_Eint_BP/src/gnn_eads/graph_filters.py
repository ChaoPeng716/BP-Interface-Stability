...
def isomorphism_test(graph: Data, graph_list: list, eps: float=0.05) -> bool:
    ...
    label = graph.label
    # fliter_list = []
    # with open('/home/nic/gnn_BP_tail/notebooks/GNN_training/fliter.txt', 'r') as file:
    #     for line in file:
    #         fliter_list.append(line.strip())
    # if label in fliter_list:
    #     return False
    energy = graph.y
    for rival_graph in graph_list:
        if label == rival_graph.label :
        # if label == rival_graph.label and np.abs(energy - rival_graph.y) < eps:
            print(f"Graph with  {label} and energy {energy} eV is isomorphic to another graph and will be excluded.")
            return False
        else:
            continue
    return True
