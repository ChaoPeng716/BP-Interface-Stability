...
def violinplot_family(model,
                      ...
                      dataset_labels: list,
                      device :str="cpu"):
    """Generate violinplot sorted by chemical family.  change constants.
...
def DFTvsGNN_plot2(model,
    ...
    params = {'mathtext.default': 'regular' }          
    plt.rcParams.update(params)
    ax.set_ylabel('$\mathit{E}_{GNN}$ per atom / eV')
    ax.set_xlabel('$\mathit{E}_{DFT}$ per atom / eV')
    predict_lst = []
    true_lst = []
    model.eval()
    for batch in loader:        
        ...
        true_lst += batch.y
        atoms_num = [graph.atoms_num.item() for graph in loader.dataset]  #add
    predict_lst = to_scal(predict_lst)
    true_lst = to_scal(true_lst)
    error = [(predict_lst[i] * std_tv)/atoms_num - (true_lst[i] * std_tv)/atoms_num  for i in range(len(true_lst))]
    abs_error = [abs(error[i]) for i in range(len(error))]
    MAE = np.mean(abs_error) 
    R2 = r2_score((mean_tv + true_lst * std_tv)/atoms_num, (mean_tv + predict_lst * std_tv)/atoms_num)  
    s_error = np.std(error)
    ax.scatter((mean_tv + true_lst * std_tv)/atoms_num, (mean_tv + predict_lst * std_tv)/atoms_num, 
               s=15, marker="o", alpha=0.7, c="#66b3ff", edgecolors="none", linewidths=0.3, zorder=2)
    ax.yaxis.set_major_locator(MaxNLocator(5)) 
    ...
    # info = 'MAE = {:.4f} eV\n$\mathit{{s}}_{{error}}$ = {:.4f} eV\n'.format(MAE, s_error)
    info = f'MAE = {MAE:.4f} eV\nR2 = {R2:.4f}'
    ax.annotate(info, [0.50, 0.15], xycoords='axes fraction')  
    # l, b, h, w = .25, .6, .2, .2
...
def DFTvsGNN_plot(model,
    ...
    #print(MAE)
    s_error = np.std(error)
    R2 = r2_score(true_lst * std_tv + mean_tv, predict_lst * std_tv + mean_tv)  # Calculate R2 score
    #print(s_error)
    ...
    ax.fill_between(x, -70, -65, where=(x < -65.0) & (x > - 70.0), alpha=0.8, color=zoomcol, zorder=0)
    # info = 'MAE = {:.4f} eV\n$\mathit{{s}}_{{error}}$ = {:.4f} eV\n'.format(MAE, s_error)
    # info = f'MAE = {MAE:.4f} eV\n$\mathit{{s}}_{{error}}$ = {s_error:.4f} eV\nR2 = {R2:.4f}'
    info = f'MAE = {MAE:.4f} eV\nR2 = {R2:.4f}'
    ax.annotate(info, [0.50, 0.15], xycoords='axes fraction')  
    # l, b, h, w = .25, .6, .2, .2
    # ax2 = fig.add_axes([l, b, w, h])
...
def DFTvsGNN_plot_train(model,
    ...
    atoms_num = [graph.atoms_num.item() for graph in loader.dataset]  #add
    true_lst = to_scal(true_lst)
    error = [predict_lst[i] * std_tv/atoms_num - true_lst[i] * std_tv/atoms_num  for i in range(len(true_lst))]  #xiugai 
    abs_error = [abs(error[i]) for i in range(len(error))]
    MAE = np.mean(abs_error) 
...
def pred_real(model,
    ...
    ax2.set_xlabel('DFT Energy Per Atom / eV')
    for axis, loader, name in zip((ax1, ax2, ax3),
        ...
        predict_lst = to_scal(predict_lst)
        true_lst = to_scal(true_lst)
        true_lst_lst.append(true_lst/atoms_num)
        axis.scatter(mean_tv + predict_lst * std_tv, mean_tv + true_lst * std_tv, s=5)
        axis.set_title(name)
        #axis.grid()
        error = (true_lst - predict_lst) * std_tv 
        absolute_error = abs(error)
        R2 = r2_score(true_lst * std_tv + mean_tv, predict_lst * std_tv + mean_tv)
        info = 'MAE: {:.4f} eV \nR2:{:.4f}'.format(np.mean(absolute_error), R2)
        axis.annotate(info, [0.50, 0.15], xycoords='axes fraction')
    plt.ioff()
...
def label_dist_train_val_test(mean_tv: float, std_tv: float, train_loader, val_loader, test_loader=None):
    "Plot data labels distribution in the training, validation and test sets."
    train_y = [graph.y.item() * std_tv + mean_tv for graph in train_loader.dataset]
    val_y = [graph.y.item() * std_tv + mean_tv for graph in val_loader.dataset]
    fig, ax = plt.subplots(figsize=(9/2.54, 6/2.54), dpi=300)
    sns.kdeplot(train_y, ax=ax, color="red")
    sns.kdeplot(val_y, ax=ax, color="blue")
    if test_loader != None:
        test_y = [graph.y.item() * std_tv + mean_tv for graph in test_loader.dataset]
        sns.kdeplot(test_y, ax=ax, color="orange")    
    ax.legend(["Train", "Val", "Test"], loc='upper left', bbox_to_anchor=(0.1, 1))
